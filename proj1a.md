...
shell is necessary to become proficient in this world; knowing how
the shell itself is built is the focus of this project.
...
Your basic shell, called `mash` (short for Mason Shell, naturally), is
basically an interactive loop: it repeatedly prints a prompt `mash> ` (note
...
At this point, `mash` is running, and ready to accept commands. Type away!
...
prompt is printed (`mash> `). In batch mode, no prompt should be printed.
...
call `exit(0);` in your mash source code, which then will exit the shell.
...
* `path`: The `path` command takes 0 or more arguments, with each argument
  ...
  `mash> path /bin /usr/bin`, which would add `/bin` and `/usr/bin` to the
...
Alternatively, you can use Git's awesome feature the Git tags.
Git is useful in tracking your development progress. You can use tags to checkpoint
the major milestones as you go. To do so, you can do something like the following:
